...
It explores whether *code can express cultural emotion*, specifically *tarab*—the feeling of musical ecstasy found in Arabic performances by artists such as Fairouz and Umm Kulthum.

The composition merges:
- Hijaz maqam** scale structures, coded algorithmically through Python lists and pitch shifts.  
- Trap and R&B rhythmic design**, built entirely using the `makeBeat()` function.  
...
## Screenshots
(Development view of the EarSketch project workspace showing multiple track layers and annotations.)
...
2. Copy the code from `arabian_soundscape.py` into the editor.
